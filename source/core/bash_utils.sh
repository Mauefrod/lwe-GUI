...
kill_process() {
    ...
    if ! is_process_running "$pid"; then
        log_debug "[KILL] Process $pid not running"
        return 0
    fi
    
    # Debug: Show process info before killing
    local cmd
    cmd=$(ps -p "$pid" -o cmd= 2>/dev/null || echo "unknown")
    log_debug "[KILL] Process info - PID=$pid, CMD=$cmd"
    
    while [[ $attempt -lt $max_attempts ]]; do
        log_debug "[KILL] Attempt $((attempt + 1))/$max_attempts - Sending SIG$signal to PID $pid"
        
        kill -"$signal" "$pid" 2>/dev/null || true
        sleep 0.3
        
        if ! is_process_running "$pid"; then
            log_debug "[KILL] Process $pid killed successfully with SIG$signal"
            return 0
        fi
        
        log_debug "[KILL] Process $pid still running after SIG$signal, will escalate"
        signal="KILL"
        attempt=$((attempt + 1))
    done
    
    log_error "[KILL] Failed to kill process $pid after $max_attempts attempts"
    return 1
}

# List all running processes matching a pattern (for debugging)
list_matching_processes() {
    local pattern="$1"
    
    log_debug "[PROCESS LIST] Checking for processes matching: $pattern"
    
    local pids
    pids=$(pgrep -f "$pattern" 2>/dev/null || echo "")
    
    if [[ -z "$pids" ]]; then
        log_debug "[PROCESS LIST] No processes found matching: $pattern"
        return 0
    fi
    ...
    echo "$pids" | while read -r pid; do
        ...
        log_debug "[PROCESS LIST] PID=$pid, PPID=$ppid, CMD=$cmd"
    done
}

# Kill all processes matching pattern (excludes current process)
kill_by_pattern() {
    local pattern="$1"
    local signal="${2:-TERM}"
    
    # Map signal names to numbers if needed
    local signal_num="$signal"
    case "$signal" in
        TERM|SIGTERM) signal_num="15" ;;
        KILL|SIGKILL) signal_num="9" ;;
        INT|SIGINT) signal_num="2" ;;
        HUP|SIGHUP) signal_num="1" ;;
        *) signal_num="$signal" ;;
    esac
    
    log_debug "Killing processes matching: $pattern (signal: SIG$signal)"
    
    # Debug: List all matching processes before killing
    list_matching_processes "$pattern"
    
# Find matching processes first for better logging
    local pids
    pids=$(pgrep -f "$pattern" 2>/dev/null || echo "")
    ...
    log_debug "Found $count process(es) matching '$pattern': $pids"
    
    # Execute pkill
    if pkill -$signal_num --exclude=$$ -f "$pattern" 2>/dev/null; then
        log_debug "Successfully sent SIG$signal to $count process(es) matching '$pattern'"
        sleep 0.2
        
        # Verify processes are actually dead
        local remaining
        remaining=$(pgrep -f "$pattern" --exclude=$$ 2>/dev/null | wc -l)
        log_debug "Remaining processes after SIG$signal: $remaining"
        return 0
    else
        log_debug "Failed to kill processes matching '$pattern' (may already be dead)"
        return 0
    fi
}
