...
kill_previous_engine() {
    ...
    # Try SIGTERM first
    if kill_by_pattern "linux-wallpaperengine" "TERM" 2>/dev/null; then
        log "Sent SIGTERM to linux-wallpaperengine processes"
        sleep 1
    fi
    
    # Check if still running and escalate to SIGKILL
    if pgrep -f "linux-wallpaperengine" >/dev/null 2>&1; then
        log "linux-wallpaperengine still running, escalating to SIGKILL"
        kill_by_pattern "linux-wallpaperengine" "KILL" 2>/dev/null || true
        sleep 0.5
    else
        log "linux-wallpaperengine has been terminated"
    fi
}
...
cmd_stop() {
    ...
    # First, try to kill the engine using the saved PID (most precise)
    log "[STOP] Attempting to kill engine using saved PID..."
    if [[ -f "$ENGINE_STATE_FILE.pid" ]]; then
        engine_pid=$(cat "$ENGINE_STATE_FILE.pid" 2>/dev/null || echo "")
        if [[ -n "$engine_pid" ]]; then
            log_debug "[STOP] Found saved engine PID: $engine_pid"
            if is_process_running "$engine_pid"; then
                log "[STOP] Killing engine process (PID $engine_pid) with SIGTERM"
                kill_process "$engine_pid"
                processes_killed=$((processes_killed + 1))
                sleep 0.5
                
                # Check if still running and escalate
                if is_process_running "$engine_pid"; then
                    log_debug "[STOP] Engine PID $engine_pid still running, escalating to SIGKILL"
                    kill -9 "$engine_pid" 2>/dev/null || true
                    sleep 0.3
                fi
            else
                log_debug "[STOP] Engine PID $engine_pid is not running"
            fi
        fi
        rm -f "$ENGINE_STATE_FILE.pid"
    else
        log_debug "[STOP] No saved engine PID file found"
    fi
    
    # Fallback: Kill by pattern (catches any remaining processes)
    log_debug "[STOP] Fallback: checking for remaining linux-wallpaperengine processes..."
    if pgrep -f "linux-wallpaperengine" >/dev/null 2>&1; then
        log_debug "[STOP] Found remaining linux-wallpaperengine processes, sending SIGTERM"
        kill_by_pattern "linux-wallpaperengine" "TERM"
        processes_killed=$((processes_killed + 1))
        sleep 1
        
        # Check if still running and escalate
        if pgrep -f "linux-wallpaperengine" >/dev/null 2>&1; then
            log_debug "[STOP] linux-wallpaperengine still running, escalating to SIGKILL"
            kill_by_pattern "linux-wallpaperengine" "KILL"
            sleep 0.5
        fi
    else
        log_debug "[STOP] No remaining linux-wallpaperengine processes found"
    fi
    ...
    if [[ -f "$PID_FILE" ]]; then
        ...
        if [[ -n "$loop_pid" ]]; then
            log_debug "[STOP] Found loop PID file: $loop_pid"
            if is_process_running "$loop_pid"; then
                log "[STOP] Killing loop process with PID: $loop_pid"
                kill_process "$loop_pid"
                processes_killed=$((processes_killed + 1))
            else
                log_debug "[STOP] Loop process $loop_pid is not running"
            fi
        fi
        rm -f "$PID_FILE"
    else
        log_debug "[STOP] No PID file found"
    fi
    
    # Final cleanup: kill any remaining main.sh instances (excluding current process)
    log "[STOP] Checking for remaining main.sh instances..."
    if pgrep -f "bash.*main.sh" --exclude=$$ >/dev/null 2>&1; then
        log_debug "[STOP] Found remaining main.sh instances, sending SIGTERM"
        kill_by_pattern "bash.*main.sh" "TERM"
        processes_killed=$((processes_killed + 1))
        sleep 1
        
        if pgrep -f "bash.*main.sh" --exclude=$$ >/dev/null 2>&1; then
            log_debug "[STOP] main.sh instances still running, escalating to SIGKILL"
            kill_by_pattern "bash.*main.sh" "KILL"
        fi
    else
        log_debug "[STOP] No remaining main.sh instances found"
    fi
    
    # Clear state files
    log "[STOP] Clearing state files..."
    rm -f "$PREV_WINDOWS_FILE" "$ENGINE_STATE_FILE.pid" 2>/dev/null || true
    
    log "[STOP] Stop command completed - killed $processes_killed process group(s)"
    return 0
}
